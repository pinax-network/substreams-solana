...
fn process_instruction(ix: &InstructionView) -> Option<pb::Instruction> {
    ...
    if program_id != &plasma::PROGRAM_ID {
        return None;
    }

    match plasma::instructions::unpack(ix.data()) {
        Ok(plasma::instructions::PlasmaInstruction::Swap(event)) => {
            let side = match event.side {
                plasma::instructions::Side::Buy => 0,
                plasma::instructions::Side::Sell => 1,
            };
            let (swap_type, amount, limit_amount) = match event.swap_type {
                plasma::instructions::SwapType::ExactIn { amount_in, min_amount_out } => (0, amount_in, min_amount_out),
                plasma::instructions::SwapType::ExactOut { amount_out, max_amount_in } => (1, amount_out, max_amount_in),
            };
            Some(pb::Instruction {
