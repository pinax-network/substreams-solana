...
pub fn process_events(tables: &mut substreams_database_change::tables::Tables, clock: &Clock, events: &pb::Events) {
    ...
    for transaction in events.transactions.iter() {
        ...
        for (i, post_balance) in transaction.post_balances.iter().enumerate() {
            ...
            system_post_balances_per_block.insert(key, (post_balance, i));
        }
        for (i, pre_balance) in transaction.pre_balances.iter().enumerate() {
            ...
            if !system_pre_balances_per_block.contains_key(&key) {
                system_pre_balances_per_block.insert(key, (pre_balance, i));
            }
        }
    }
    let mut skipped = 0;
    for (post_balance, _) in system_post_balances_per_block.values() {
        if let Some((pre_balance, _)) = system_pre_balances_per_block.get(&(post_balance.account.as_slice())) {
...
fn handle_balances(tables: &mut substreams_database_change::tables::Tables, clock: &Clock, data: &pb::Balance) {
