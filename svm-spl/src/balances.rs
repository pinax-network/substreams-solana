    ...
    for transaction in events.transactions.iter() {
        ...
        for (i, balance) in transaction.pre_token_balances.iter().enumerate() {
            ...
            if !pre_token_balances_per_block.contains_key(&key) {
                pre_token_balances_per_block.insert(key, (balance, i));
            }
        }
        for (i, balance) in transaction.post_token_balances.iter().enumerate() {
            let key = (balance.account.as_slice(), balance.mint.as_slice());
            post_token_balances_per_block.insert(key, (balance, i));
        }
    }
    let mut skipped = 0;
    for (post_balance, _) in post_token_balances_per_block.values() {
        // if balance not changed in the block - no need to include it - skip it
        if let Some((pre_balance, _)) = pre_token_balances_per_block.get(&(post_balance.account.as_slice(), post_balance.mint.as_slice())) {
...
fn handle_token_balances(tables: &mut substreams_database_change::tables::Tables, clock: &Clock, data: &pb::TokenBalance) {
