...
SELECT
    ...
    -- swaps --
    argMinState(s.price, toUInt64(s.block_num))                AS open0,
    quantileDeterministicState(s.price, toUInt64(s.block_num)) AS quantile0,
    argMaxState(s.price, toUInt64(s.block_num))                AS close0,

    -- volume --
    sum(abs(s.amount0))        AS gross_volume0,
    sum(abs(s.amount1))        AS gross_volume1,
    sum(s.amount0)             AS net_flow0,
    sum(s.amount1)             AS net_flow1,
